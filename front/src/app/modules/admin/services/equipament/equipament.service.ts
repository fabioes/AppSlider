...
export class EquipamentService {
  ...
  public getByEstablishment(establishments: Array<Model.App.Business>): Observable<Array<Model.App.Equipament>> {
    let params = new HttpParams();
    let guids:string[] = [];
    for (let i of establishments) {
      guids.push(i.id);
    }
    // params = params.append('establishmentIds', guids.join(', '));
    console.log(guids);
    return this.httpHelper.HttpPost<Model.Core.ApiResultList<Model.App.Equipament>>(environment.apiConfig.apiRoutes.equipament.getByEstablishments, guids)
      .pipe(map(res => res.items));
  }
